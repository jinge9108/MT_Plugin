...
public class AIHelper {
    ...
    @NonNull
    public static JSONArray createAnalysisMessages(
            @NonNull PluginContext context,
            @NonNull String code,
        ...
        appendMessage(messages, "system", prompt);
        appendMessage(messages, "user", "请分析以下代码：\n\n" + code);
        return messages;
    }
    ...
    public static class AIResponse {
        ...
        public AIResponse(@NonNull String reasoning, @NonNull String content) {
    ...
    public interface StreamCallback {
        void onStream(@NonNull String reasoning, @NonNull String content);
    }

    @NonNull
    public static String chatWithMessages(
            @NonNull PluginContext context,
            @NonNull JSONArray messages,
            @Nullable PluginEditText thinkingEdit,
            @Nullable PluginDialog dialog,
            boolean showThinking) throws Exception {

        AIResponse response = performChatRequest(context, messages, thinkingEdit, showThinking, null);
        return response.content;
    }

    @NonNull
    public static AIResponse chatWithMessagesDetailed(
            @NonNull PluginContext context,
            @NonNull JSONArray messages,
            boolean showThinking) throws Exception {
        return performChatRequest(context, messages, null, showThinking, null);
    }
    ...
    public static AIResponse chatWithMessagesDetailed(
            ...
            @Nullable StreamCallback streamCallback) throws Exception {
        return performChatRequest(context, messages, null, showThinking, streamCallback);
    }

    @NonNull
    private static AIResponse performChatRequest(
            @NonNull PluginContext context,
            @NonNull JSONArray messages,
            @Nullable PluginEditText thinkingEdit,
            boolean showThinking,
                    ...
                    if (choices != null && choices.length() > 0) {
                        ...
                        if (delta != null) {
                            ...
                            if (streamCallback != null) {
                                streamCallback.onStream(fullReasoning.toString(), fullContent.toString());
                            }
                        }
                    }
                } catch (Exception e) {
        ...
        if (fullContent.length() == 0) {
            if (fullReasoning.length() > 0) {
                fullContent.append(fullReasoning);
            } else {
                throw new Exception("AI API返回空结果");
            }
        }

        return new AIResponse(fullReasoning.toString(), fullContent.toString());
    }
    ...
    public static String[] analyzeCodeWithAI(
            ...
            boolean showThinking,
            @Nullable String customPrompt) throws Exception {
        JSONArray messages = createAnalysisMessages(context, code, customPrompt);
        AIResponse response = performChatRequest(context, messages, thinkingEdit, showThinking, null);
